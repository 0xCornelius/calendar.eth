...
contract CalendarManager is IERC721Receiver {
    ...
    function createEvent(address[] memory invitees, uint256 date, string memory description) public returns(uint256) {
        for (uint i; i < invitees.length; i++) {
            if(!EnumerableSet.contains(allowances[invitees[i]], msg.sender)) {
                revert("Invitee did not approve");
            }
        }        
        uint256 eventId = eventContract.createEvent(date, invitees, description, msg.sender);
